...
const TrackClaimPage = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center"><Search className="mr-2"/>Track Your Claim</CardTitle>
          <CardDescription>Enter your Claim ID to check the current status of your application.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex space-x-2">
            ...
            <Button onClick={handleTrackClaim} disabled={isLoading}>
              {isLoading ? <Loader2 className="animate-spin h-4 w-4" /> : "Track"}
            </Button>
          </div>
            ...
            <div className="mt-6 border-t pt-6">
              ...
              <div className="grid grid-cols-2 gap-4 mt-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{new Date(claimDetails.submittedDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Last Updated</p>
                  <p className="font-medium">{new Date(claimDetails.lastUpdate).toLocaleDateString()}</p>
                </div>
              </div>
            </div>
          )}
