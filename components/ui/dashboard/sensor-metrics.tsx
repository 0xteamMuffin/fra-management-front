...
export function SensorMetrics({ sensors }: SensorMetricsProps) {
  const getAverageValue = (field: keyof SensorData) => {
    ...
    const sum = onlineSensors.reduce(
      (acc, sensor) => acc + (sensor[field] as number),
      0,
    );
    ...
    <div className="space-y-4">
      <Card className="border-border bg-white">
        <CardHeader>
          ...
          <CardDescription>
            Average readings from active sensors
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-lg font-bold">
                  {avgSoilMoisture.toFixed(1)}%
                </div>
                <div
                  className={`text-xs ${getSoilMoistureStatus(avgSoilMoisture).color}`}
                >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-lg font-bold">
                  {avgGroundwater.toFixed(1)}m
                </div>
                <div
                  className={`text-xs ${getGroundwaterStatus(avgGroundwater).color}`}
                >
            ...
            <Progress
              value={Math.min(100, (avgGroundwater / 20) * 100)}
              className="h-2"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Progress
              value={Math.min(100, (avgTemperature / 50) * 100)}
              className="h-2"
            />
          </div>
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div
                    ...
                    style={{
                      width: `${Math.max(8, (sensor.batteryLevel / 100) * 64)}px`,
                    }}
                  />
                </div>
              </div>
