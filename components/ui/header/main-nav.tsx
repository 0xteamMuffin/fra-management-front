...
export function MainNav({ variant = "desktop", onClick }: MainNavProps) {
  ...
  const dashboardUrl =
    user?.role === UserRole.VillagePerson ? "/dashboard/u" : "/dashboard/gp";
    ...
    >
      ...
      <nav
        ...
        className={cn(
          ...
          variant === "desktop"
            ? "gap-6"
            : "flex-col items-start gap-3 w-full px-3",
        )}
      >
        {navLinks.map((l) => {
          ...
          return (
            <Link
              ...
              href={l.href}
              onClick={onClick}
              className={cn("w-full", variant === "desktop" && "w-auto")}
            >
              <Button
                variant="ghost"
                size="sm"
