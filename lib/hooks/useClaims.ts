...
export function useClaims(options: UseClaimsOptions = {}) {
  ...
  const { execute: verifyClaim, isLoading: isVerifying } = useApi(
    claimsService.verifyClaim,
  );

  const { execute: forwardClaim, isLoading: isForwarding } = useApi(
    claimsService.forwardClaim,
  );

  const { execute: approveClaim, isLoading: isApproving } = useApi(
    claimsService.approveClaim,
  );

  const { execute: rejectClaim, isLoading: isRejecting } = useApi(
    claimsService.rejectClaim,
  );

  const { execute: createClaim, isLoading: isCreating } = useApi(
  ...
  useEffect(() => {
    if (rawClaims) {
      ...
      if (villageId) {
        filteredClaims = rawClaims.filter(
          (claim) => claim.villageId === villageId,
        );
      }

      const converted = filteredClaims.map((claim) =>
        convertFRAClaimToClaimRow(claim, user?.role),
      );
  ...
  // Forward a claim
  const handleForwardClaim = useCallback(
    async (claimId: string, remarks: string) => {
